...
- **摇杆映射**: PS2 摇杆满幅映射 1000-2000μs (不设限幅)。LOCKED 状态下也输出 PWM, 方便 ESC 编程/校准。

## Known Hardware Limitations

### C06B 板 PWM 信号注意事项
- **H6/H7 接口串联 220Ω 电阻**: PB8/PB9 经 R29/R31 (220Ω) 连接到舵机接口排针。这是舵机保护电路, 对 50Hz 信号本身不影响, 但会限制驱动电流。
- **GPIO 输出 3.3V 电平**: STM32F103 所有 GPIO 为 3.3V。标准 RC 接收机输出 5V PWM, ZTW Seal G2 等船用电调可能对 3.3V PWM 检测不理想。
- **排查项**: 如 ESC 对 PWM 不响应: ① 短接 220Ω (飞线/焊锡); ② 加 3.3V→5V 电平转换模块; ③ 确认 ESC 支持 3.3V 输入。

### 已知 Bug (已修复)
- **`escInit()` CRH 寄存器位偏移错误**: 初始代码配置了 PB12/PB13 而非 PB8/PB9。PB8/PB9 停留在浮空输入态, TIM4 PWM 无法输出到引脚。修复: `(0xF << 16|20)` → `(0xF << 0|4)`。
