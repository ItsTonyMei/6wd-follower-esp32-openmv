...
| **ZTW Seal G2 无刷电调** | 🔴 调试中 | 已采购双路 (已设双向模式 via FPV 遥控器), STM32 PWM 校准存在信号识别问题 |

### 待完成

| 优先级 | 任务 | 说明 |
|--------|------|------|
| 🔴 高 | 解决 STM32→ZTW ESC PWM 信号兼容 | C06B 220Ω串联 + 3.3V 电平可能导致 ESC 校准失败 (已确认中位 1500μs 不被识别) |
| 🔴 高 | 坦克混控实车验证 | 电调信号问题解决后进行 |
...
| 日期 | 变更 | 原因 |
|------|------|------|
| 2026-06-02 | **STM32 PWM 输出修复** | `escInit()` CRH 寄存器位偏移错误修正 (PB12/13→PB8/9); 摇杆改为满幅 1000-2000μs |
| 2026-06-02 | **ZTW Seal G2 PWM 信号兼容问题排查** | 识别到 C06B H6/H7 220Ω 串联 + 3.3V 电平可能导致船用电调校准失败; FPV 遥控器使用普通 PWM 通道(非 SBUS) |
